// ==UserScript==
// @name         三鷹市テニスコート空き状況チェッカー（ベータ版）
// @namespace    https://yoyaku-mitaka.jp/beta
// @version      0.9.0
// @description  【ベータ版】現行版と併用可。チップから予約直前の時間帯選択画面へジャンプする機能などを試験実装中。
// @author       you
...
(function () {
  ...
  const STORAGE_KEY = 'mtc_targets_v1_beta';
  ...
  const STATUS_CLASS = {
    [STATUS.NO_SCHEDULE]: 'mtc2-cell-none',
    [STATUS.FULL]:        'mtc2-cell-full',
    [STATUS.AVAILABLE]:   'mtc2-cell-available',
    [STATUS.PARTIALLY_AVAILABLE]: 'mtc2-cell-partial',
  };
  ...
  const LAST_JUMPED_FACILITY_KEY = 'mtc2_last_jumped_facility_id';
  ...
  async function resetSiteSessionIfSwitchingFacility(facilityId) {
    ...
    try {
      ...
      console.log('[mitaka-tennis-checker-beta] /reservation reloaded, fresh CSRF token:', freshToken ? 'obtained' : 'not found');
      setLastJumpedFacilityId(facilityId);
      return freshToken;
    } catch (e) {
      console.error('[mitaka-tennis-checker-beta] /reservation reload failed', e);
    }
    setLastJumpedFacilityId(facilityId);
  }
  ...
  async function jumpToBooking(ti, dateStr) {
    ...
    try {
      ...
      try {
        ...
        console.error('[mitaka-tennis-checker-beta] sessionStorage write failed', e);
      }
      ...
      if (data && data.success && data.redirect_url) {
        ...
        // サーバー側のセッション書き込みが完了する前に次の画面を開いてしまう
        // 競合を疑い、念のため少し待ってから開く
      ...
      console.error('[mitaka-tennis-checker-beta] jumpToBooking failed', e);
  ...
  function injectStyles() {
    const css = `
      ...
      #mtc2-panel h2, #mtc2-settings h2 { margin: 0 0 8px; font-size: 15px; }
      #mtc2-panel .mtc2-close, #mtc2-settings .mtc2-close {
        position: absolute; top: 8px; right: 10px; cursor: pointer;
        font-size: 20px; background: none; border: none;
      }
      #mtc2-panel .mtc2-toolbar, #mtc2-settings .mtc2-toolbar { display: flex; gap: 6px; margin-bottom: 8px; flex-wrap: wrap; }
      #mtc2-panel button.mtc2-btn, #mtc2-settings button.mtc2-btn {
        background: #2e7d32; color: #fff; border: none; border-radius: 6px;
        padding: 5px 10px; cursor: pointer; font-size: 12px;
      }
      #mtc2-panel button.mtc2-btn.secondary, #mtc2-settings button.mtc2-btn.secondary { background: #666; }
      #mtc2-panel .mtc2-tabs-label { font-size: 11px; color: #888; margin-bottom: 2px; }
      #mtc2-panel .mtc2-tabs { display: flex; gap: 4px; flex-wrap: wrap; margin-bottom: 8px; }
      #mtc2-panel .mtc2-tab {
        border: 1px solid #bbb; background: #f5f5f5; color: #333;
        border-radius: 999px; padding: 4px 12px; font-size: 12px; cursor: pointer;
      }
      #mtc2-panel .mtc2-tab.active { background: #2e7d32; color: #fff; border-color: #2e7d32; }
      #mtc2-panel .mtc2-monthnav {
        display: flex; align-items: center; justify-content: center; gap: 14px;
        margin-bottom: 6px;
      }
      #mtc2-panel .mtc2-monthnav button {
        border: 1px solid #bbb; background: #fff; border-radius: 6px;
        width: 30px; height: 30px; cursor: pointer; font-size: 14px;
      }
      #mtc2-panel .mtc2-monthnav .mtc2-month-select {
        font-size: 15px; font-weight: bold; text-align: center; padding: 4px 6px;
        border: 1px solid #bbb; border-radius: 6px; background: #fff;
      }
      #mtc2-panel .mtc2-timefilter {
        display: flex; align-items: center; gap: 6px; justify-content: center;
        margin-bottom: 10px; font-size: 12px; flex-wrap: wrap;
      }
      #mtc2-panel .mtc2-timefilter select {
        padding: 3px 4px; border-radius: 4px; border: 1px solid #ccc; font-size: 12px;
      }
      #mtc2-panel .mtc2-preset-row {
        display: flex; justify-content: center; gap: 6px; margin-bottom: 8px; flex-wrap: wrap;
      }
      #mtc2-panel .mtc2-preset-btn {
        background: #e8f5e9; color: #2e7d32; border: 1px solid #a5d6a7;
        border-radius: 999px; padding: 3px 10px; font-size: 11px; cursor: pointer;
      }
      #mtc2-panel .mtc2-preset-btn:active { background: #c8e6c9; }
      #mtc2-panel .mtc2-legend { font-size: 11px; color: #555; text-align: center; margin-bottom: 8px; }
      #mtc2-panel .mtc2-filter-summary {
      ...
      #mtc2-panel .mtc2-fetched-at { font-size: 10px; color: #888; }
      #mtc2-panel .mtc2-facility-legend {
        display: flex; flex-wrap: wrap; justify-content: center; gap: 10px;
        margin-bottom: 8px; font-size: 11px; color: #333;
      }
      #mtc2-panel .mtc2-legend-item { display: flex; align-items: center; gap: 4px; }
      #mtc2-panel .mtc2-legend-dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
      #mtc2-panel .mtc2-calendar { width: 100%; border-collapse: collapse; table-layout: fixed; }
      #mtc2-panel .mtc2-calendar th {
        font-size: 11px; color: #666; padding: 4px 0; border-bottom: 1px solid #ddd;
      }
      #mtc2-panel .mtc2-calendar th.mtc2-sun { color: #c62828; }
      #mtc2-panel .mtc2-calendar th.mtc2-sat { color: #1565c0; }
      #mtc2-panel .mtc2-calendar td {
        border: 1px solid #eee; text-align: left; vertical-align: top;
        min-height: 54px; padding: 2px; font-size: 10px; cursor: pointer;
      }
      #mtc2-panel .mtc2-calendar td.mtc2-empty { background: #fafafa; border: none; cursor: default; }
      #mtc2-panel .mtc2-day-num { font-size: 11px; color: #888; text-align: right; padding-right: 2px; }
      #mtc2-panel .mtc2-day-num.mtc2-sun { color: #c62828; }
      #mtc2-panel .mtc2-day-num.mtc2-sat { color: #1565c0; }
      #mtc2-panel .mtc2-cal-chip {
        border-radius: 3px; padding: 1px 4px; margin-bottom: 2px; font-size: 9px; line-height: 1.35;
        white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
      }
      #mtc2-panel .mtc2-chip-full { background: #c8e6c9; color: #1b5e20; }
      #mtc2-panel .mtc2-chip-partial { background: #fff3cd; color: #7a5b00; }
      #mtc2-panel .mtc2-chip-more { background: #eee; color: #666; text-align: center; }
      #mtc2-panel .mtc2-cal-none { color: #bbb; font-size: 11px; text-align: center; padding-top: 8px; }
      #mtc2-panel .mtc2-weekday-filter {
        display: flex; justify-content: center; gap: 6px; margin-bottom: 8px; font-size: 11px;
      }
      #mtc2-panel .mtc2-weekday-filter label { display: flex; align-items: center; gap: 2px; cursor: pointer; }
      #mtc2-panel td.mtc2-cell-available { background: #e8f5e9; }
      #mtc2-panel td.mtc2-cell-partial { background: #fff8e1; }
      #mtc2-panel td.mtc2-cell-full { background: #fdecea; }
      #mtc2-panel td.mtc2-cell-none { background: #fafafa; }
      #mtc2-panel .mtc2-loading { padding: 40px; text-align: center; color: #888; }
      #mtc2-panel .mtc2-detail-date {
        font-weight: bold; margin: 10px 0 4px; display: flex; align-items: center; gap: 6px;
      }
      #mtc2-panel .mtc2-chips { display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 8px; }
      #mtc2-panel .mtc2-chip {
      ...
      #mtc2-panel .mtc2-chip b { font-weight: 700; }
      #mtc2-panel .mtc2-facility-title {
      ...
      #mtc2-settings h2 { margin: 0 0 10px; font-size: 16px; }
      #mtc2-settings .mtc2-row {
        display: grid; grid-template-columns: 36px 55px 55px 1fr 1fr 90px;
        gap: 6px; margin-bottom: 6px; align-items: center;
      }
      #mtc2-settings .mtc2-row input[type="checkbox"] { width: 18px; height: 18px; justify-self: center; }
      #mtc2-settings .mtc2-row.mtc2-row-disabled { opacity: 0.45; }
      #mtc2-settings input {
        width: 100%; box-sizing: border-box; padding: 4px 6px;
        border: 1px solid #ccc; border-radius: 4px; font-size: 12px;
      }
      #mtc2-settings .mtc2-row-head { font-size: 11px; color: #888; }
    `;
  ...
  function buildCalendarMulti(indices, year, month, startMin, endMin, selectedDow) {
    ...
    let html = '<table class="mtc2-calendar"><thead><tr>';
    dowList.forEach((i) => {
      const cls = i === 0 ? 'mtc2-sun' : i === 6 ? 'mtc2-sat' : '';
      html += `<th class="${cls}">${DOW[i]}</th>`;
    });
    html += '</tr></thead><tbody><tr>';

    for (let d = 0; d < startDow; d++) {
      if (selectedDow.has(d)) html += '<td class="mtc2-empty"></td>';
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      if (selectedDow.has(dow)) {
        ...
        const cls = STATUS_CLASS[info.status] || 'mtc2-cell-none';
        const dowClass = dow === 0 ? 'mtc2-sun' : dow === 6 ? 'mtc2-sat' : '';
        const shown = info.chips.slice(0, MAX_INLINE_CHIPS_MULTI);
        const overflow = info.chips.length - shown.length;

        let chipsHtml = '';
        if (info.noneText) {
          chipsHtml = `<div class="mtc2-cal-none">${info.noneText}</div>`;
        } else {
          shown.forEach((c) => {
            const chipClass = c.kind === 'full' ? 'mtc2-chip-full' : 'mtc2-chip-partial';
            chipsHtml += `<div class="mtc2-cal-chip ${chipClass}" style="border-left:3px solid ${c.color};">${c.label} ${c.timeText} ${c.nText}</div>`;
          });
          if (overflow > 0) {
            chipsHtml += `<div class="mtc2-cal-chip mtc2-chip-more">+${overflow}件</div>`;
          }
        }

        html += `<td class="${cls}" data-date="${dateStr}">` +
          `<div class="mtc2-day-num ${dowClass}">${day}</div>` +
          chipsHtml +
          '</td>';
      }
    ...
    while (dow !== 0 && dow !== 7) {
      if (selectedDow.has(dow)) html += '<td class="mtc2-empty"></td>';
      dow++;
    }
    html += '</tr></tbody></table>';
    return html;
  ...
  function buildMonthDetailMulti(indices, year, month, startMin, endMin, selectedDow) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      html += `<div class="mtc2-detail-date"><span style="color:${dowColor};">${month}/${day}(${DOW[dow]})</span></div><div class="mtc2-chips">`;
      info.chips.forEach((c) => {
        html += `<span class="mtc2-chip mtc2-chip-jump" data-ti="${c.ti}" data-date="${dateStr}" style="border-left:3px solid ${c.color};cursor:pointer;" title="予約の時間帯選択画面へ進む"><b>${c.label}</b> ${c.timeText}（${c.nText}）</span>`;
      });
      html += '</div>';
    }
  ...
  // ============================================================
  const FILTER_STATE_KEY = 'mtc_filter_state_v1_beta';
  ...
  function buildFilterSummaryHtml(indices) {
    ...
    return (
      `<div class="mtc2-filter-summary" id="mtc2-filter-summary">` +
      `<div>${monthText}　施設: ${names}</div>` +
      `<div>時間帯: ${timeText}　曜日: ${dowText}</div>` +
      (fetchedText ? `<div class="mtc2-fetched-at">${fetchedText}</div>` : '') +
      `</div>`
    );
  }

  async function renderCalendarView(panel) {
    const body = panel.querySelector('#mtc2-body');
    if (Object.keys(lastData).length === 0) {
      body.innerHTML = '<div class="mtc2-loading">読み込み中...</div>';
    ...
    const select = panel.querySelector('#mtc2-month-select');
    ...
    const legendHtml =
      indices.length > 1
        ? `<div class="mtc2-facility-legend">${indices
            .map(
              (ti) =>
                `<span class="mtc2-legend-item"><span class="mtc2-legend-dot" style="background:${facilityColor(ti)};"></span>${TARGETS[ti] ? TARGETS[ti].name : ''}</span>`
            )
            .join('')}</div>`
        : '';
    ...
    body.innerHTML =
      `<div class="mtc2-legend">凡例：緑=全面空き　黄=一部空き　－対象外／セルをタップで詳細</div>` +
      `<div id="mtc2-capture-area">` +
  ...
  function renderTabs(panel) {
    const tabs = panel.querySelector('#mtc2-tabs');
    const indices = enabledIndices();
    tabs.innerHTML = indices
      .map(
        (i) =>
          `<button type="button" class="mtc2-tab ${state.selectedTi.has(i) ? 'active' : ''}" data-i="${i}">${TARGETS[i].name}</button>`
      )
      .join('');
    tabs.querySelectorAll('.mtc2-tab').forEach((btn) => {
  ...
  function bindDetailChipJump(panel) {
    const body = panel.querySelector('#mtc2-body');
    body.querySelectorAll('.mtc2-chip-jump').forEach((chip) => {
  ...
  function bindCellTapPopover(panel) {
    const body = panel.querySelector('#mtc2-body');
    body.querySelectorAll('.mtc2-calendar td[data-date]').forEach((td) => {
      td.addEventListener('click', (e) => {
        ...
        indices.forEach((ti) => {
          ...
          inner += `<div style="font-weight:bold;color:${facilityColor(ti)};margin-top:4px;display:flex;align-items:center;justify-content:space-between;gap:6px;">` +
            ...
            `<button type="button" class="mtc2-daypop-jump" data-ti="${ti}" data-date="${dateStr}" style="background:#2e7d32;color:#fff;border:none;border-radius:4px;padding:2px 8px;font-size:10px;cursor:pointer;">予約画面へ</button>` +
            `</div>`;
          inner += '<div class="mtc2-chips" style="margin-bottom:2px;">';
          Array.from(slotMap.entries())
            .sort(([a], [b]) => (a < b ? -1 : 1))
            .forEach(([time, courts]) => {
              inner += `<span class="mtc2-chip"><b>${time}</b>（${courts.length}面：${courts.join('・')}）</span>`;
            });
          inner += '</div>';
        });
        if (!any) {
        ...
        inner +=
          '<button type="button" id="mtc2-daypop-close" style="margin-top:4px;background:#666;color:#fff;border:none;border-radius:4px;padding:3px 10px;font-size:11px;cursor:pointer;">閉じる</button>';
        pop.innerHTML = inner;
        ...
        pop.querySelector('#mtc2-daypop-close').addEventListener('click', () => {
          pop.style.display = 'none';
        });
        pop.querySelectorAll('.mtc2-daypop-jump').forEach((btn) => {
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      <button class="mtc2-close" id="mtc2-close-btn">×</button>
      <h2>三鷹市テニスコート空き状況</h2>
      <div class="mtc2-toolbar">
        <button class="mtc2-btn" id="mtc2-refresh-btn">再取得</button>
        <button class="mtc2-btn secondary" id="mtc2-copy-btn">LINE用に画像コピー</button>
        <button class="mtc2-btn secondary" id="mtc2-settings-btn">⚙ 施設設定</button>
      </div>
      <div class="mtc2-tabs-label">施設（複数選択可）</div>
      <div class="mtc2-tabs" id="mtc2-tabs"></div>
      <div class="mtc2-monthnav">
        <button type="button" id="mtc2-prev-month">◀</button>
        <select class="mtc2-month-select" id="mtc2-month-select"></select>
        <button type="button" id="mtc2-next-month">▶</button>
      </div>
      <div class="mtc2-timefilter">
        時間帯：
        <select id="mtc2-start-time">${timeOptions}</select>
        〜
        <select id="mtc2-end-time">${timeOptions}</select>
        <button class="mtc2-btn secondary" id="mtc2-time-reset" type="button">指定なし</button>
      </div>
      <div class="mtc2-preset-row" id="mtc2-time-presets">
        <button class="mtc2-preset-btn" type="button" data-start="06:00" data-end="12:00">午前</button>
        <button class="mtc2-preset-btn" type="button" data-start="12:00" data-end="17:00">午後</button>
        <button class="mtc2-preset-btn" type="button" data-start="17:00" data-end="19:00">夕方</button>
        <button class="mtc2-preset-btn" type="button" data-start="19:00" data-end="24:00">ナイター(19時〜)</button>
      </div>
      <div class="mtc2-weekday-filter" id="mtc2-weekday-filter">
        ${DOW.map((d, i) => `<label><input type="checkbox" data-dow="${i}" ${state.selectedDow.has(i) ? 'checked' : ''}>${d}</label>`).join('')}
      </div>
      <div class="mtc2-preset-row" id="mtc2-dow-presets">
        <button class="mtc2-preset-btn" type="button" data-dow-preset="weekday">平日</button>
        <button class="mtc2-preset-btn" type="button" data-dow-preset="weekend">週末</button>
        <button class="mtc2-preset-btn" type="button" data-dow-preset="all">全曜日</button>
      </div>
      <div id="mtc2-body"><div class="mtc2-loading">読み込み中...</div></div>
    `;
    document.body.appendChild(panel);

    panel.querySelector('#mtc2-start-time').value = minToTime(state.startMin);
    panel.querySelector('#mtc2-end-time').value = minToTime(state.endMin);

    panel.querySelector('#mtc2-close-btn').addEventListener('click', () => {
      panel.style.display = 'none';
    });
    panel.querySelector('#mtc2-refresh-btn').addEventListener('click', async () => {
      panel.querySelector('#mtc2-body').innerHTML = '<div class="mtc2-loading">読み込み中...</div>';
      monthListCache = {};
      await fetchAllInitial();
      renderCalendarView(panel);
    });
    panel.querySelector('#mtc2-copy-btn').addEventListener('click', copyAsImage);
    panel.querySelector('#mtc2-settings-btn').addEventListener('click', () => openSettings(panel));
    panel.querySelector('#mtc2-prev-month').addEventListener('click', () => stepMonth(panel, -1));
    panel.querySelector('#mtc2-next-month').addEventListener('click', () => stepMonth(panel, 1));
    panel.querySelector('#mtc2-month-select').addEventListener('change', (e) => {
    ...
    const applyTimeFilter = () => {
      state.startMin = timeToMin(panel.querySelector('#mtc2-start-time').value);
      state.endMin = timeToMin(panel.querySelector('#mtc2-end-time').value);
      renderCalendarView(panel);
    };
    panel.querySelector('#mtc2-start-time').addEventListener('change', applyTimeFilter);
    panel.querySelector('#mtc2-end-time').addEventListener('change', applyTimeFilter);
    panel.querySelector('#mtc2-time-reset').addEventListener('click', () => {
      panel.querySelector('#mtc2-start-time').value = '00:00';
      panel.querySelector('#mtc2-end-time').value = '24:00';
      applyTimeFilter();
    });

    panel.querySelectorAll('#mtc2-weekday-filter input[type="checkbox"]').forEach((cb) => {
    ...
    panel.querySelectorAll('#mtc2-time-presets .mtc2-preset-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
        panel.querySelector('#mtc2-start-time').value = btn.dataset.start;
        panel.querySelector('#mtc2-end-time').value = btn.dataset.end;
        applyTimeFilter();
      });
    });

    panel.querySelectorAll('#mtc2-dow-presets .mtc2-preset-btn').forEach((btn) => {
      btn.addEventListener('click', () => {
  ...
  function buildSettingsModal() {
    ...
    modal.innerHTML = `
      <button class="mtc2-close" id="mtc2-settings-close">×</button>
      <h2>監視する施設・コート設定</h2>
      ...
      <div class="mtc2-row mtc2-row-head"><div>取得</div><div>施設ID</div><div>コートID</div><div>表示名</div><div>備考</div><div>除外コート番号</div></div>
      <div id="mtc2-settings-rows"></div>
      <div class="mtc2-toolbar" style="margin-top:10px;">
        <button class="mtc2-btn secondary" id="mtc2-settings-add" type="button">＋ 行を追加</button>
        <button class="mtc2-btn" id="mtc2-settings-save" type="button">保存して再取得</button>
        <button class="mtc2-btn secondary" id="mtc2-settings-reset" type="button">初期設定に戻す</button>
      </div>
    `;
  ...
  function renderSettingsRows(modal, targets) {
    const container = modal.querySelector('#mtc2-settings-rows');
    container.innerHTML = '';
    targets.forEach((t, i) => {
      const row = document.createElement('div');
      row.className = 'mtc2-row' + (t.enabled === false ? ' mtc2-row-disabled' : '');
      row.innerHTML = `
  ...
  function readSettingsRows(modal, options) {
    options = options || {};
    const rows = Array.from(modal.querySelectorAll('#mtc2-settings-rows .mtc2-row'));
    return rows
      .map((row) => {
  ...
  function openSettings(panel) {
    ...
    settingsModal.querySelector('#mtc2-settings-close').onclick = () => {
      settingsModal.style.display = 'none';
    };
    settingsModal.querySelector('#mtc2-settings-add').onclick = () => {
      workingTargets.push({ facilityId: '', roomId: '', name: '', note: '', enabled: true, excludeCourts: [] });
      renderSettingsRows(settingsModal, workingTargets);
    };
    settingsModal.querySelector('#mtc2-settings-reset').onclick = () => {
      workingTargets = DEFAULT_TARGETS.map((t) => ({ ...t }));
      renderSettingsRows(settingsModal, workingTargets);
    };
    settingsModal.querySelector('#mtc2-settings-save').onclick = () => {
