...
// 1.5 (2026-04-30) initFacilityを週ごとに実行（ス// ==UserScript==
// @name         都立公園テニスチェッカー
// @namespace    kouen-tennis-checker
// @version      1.7
// @description  都立公園テニスコートの空き時間帯を自動収集・表示
...
(function () {
    ...
    async function runSearch() {
        ...
        for (var fi = 0; fi < enabledFacs.length; fi++) {
            ...
            for (var w = 0; w < totalWeeks; w++) {
                ...
                // 各週ごとにinitFacility（スマホ新規セッションで2週目以降0件になる対策）
                log('初期化中: ' + fac.name + ' ' + currentUseDay);
                await initFacility(fac.bldCd, fac.instCd, currentUseDay);
                ...
                try {
                    var data = await fetchWeek(fac.bldCd, fac.instCd, currentUseDay, fac.mode);
                    var count = 0;
                    ...
                    if (data.result) {
                        ...
                        log('  [DBG] data.result なし。キー=' + Object.keys(data).join(','));
                    }
                    log('  → ' + count + '件 (累計' + results.length + '件)');
    ...
    function renderResults(results) {
        ...
        if (!results.length) { el.innerHTML = '<div style="color:#999;font-size:11px;padding:8px;">空きコマなし</div>'; return; }
        ...
        var filtered = applyFilters(results);
        var body = _viewMode === 'pivot' ? renderPivot(filtered) : renderWeek(filtered);
        el.innerHTML = updBar + filterBar + ctrl + body;
        // innerHTML設定後に高さを2段階で設定（ブラウザのレイアウト完了を待つ）
        setTimeout(setResultsHeight, 100);
        setTimeout(setResultsHeight, 400);
    ...
    function setStatus(msg, color) {
        ...
        if (!el) return;
        el.textContent = msg;
        var colors = { blue:'#2196F3', green:'#4CAF50', red:'#e53935' };
        el.style.color = colors[color] || '#555';
        updateButtons();
    }
